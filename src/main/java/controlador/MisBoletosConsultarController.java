...
public class MisBoletosConsultarController {

    // --- Campos Comunes ---
    @FXML private Label labelTituloConsulta;
    @FXML private Label labelMensaje;
    @FXML private ScrollPane scrollPaneResultados;
    @FXML private VBox vboxResultados;

    // --- Contenedores y Campos Específicos ---
    @FXML private StackPane stackPaneBusqueda; // Contenedor padre
    @FXML private HBox hboxBusquedaPasajero;
    @FXML private TextField fieldInputPasajero;
    @FXML private HBox hboxBusquedaBoleto;
    @FXML private ComboBox<String> comboBoxInputBoleto;
    ...
    /**
     * Este método es llamado por MisBoletosOpcionesController para
     * indicar qué campo de búsqueda mostrar.
     */
    public void setModoBusqueda(boolean buscarPorPasajero) {
        this.buscarPorPasajero = buscarPorPasajero;
        initializeUIBasedOnMode();
    }

    /**
     * Configura la UI inicial basada en el modo de búsqueda.
     * Se llama desde setModoBusqueda después de que FXML se inicializa.
     */
    private void initializeUIBasedOnMode() {
        ocultarMensaje();
        scrollPaneResultados.setVisible(false);
        scrollPaneResultados.setManaged(false);

        if (buscarPorPasajero) {
            labelTituloConsulta.setText("CONSULTAR BOLETO POR PASAJERO");
            hboxBusquedaPasajero.setVisible(true);
            hboxBusquedaPasajero.setManaged(true);
            hboxBusquedaBoleto.setVisible(false);
            hboxBusquedaBoleto.setManaged(false);
        } else {
            labelTituloConsulta.setText("CONSULTAR BOLETO POR ID");
            hboxBusquedaPasajero.setVisible(false);
            hboxBusquedaPasajero.setManaged(false);
            hboxBusquedaBoleto.setVisible(true);
            hboxBusquedaBoleto.setManaged(true);
            cargarBoletos(); // Carga el ComboBox solo en este modo
        }
    }

    private void cargarBoletos() {
        if(comboBoxInputBoleto == null) return;
        comboBoxInputBoleto.getItems().clear();
        Boleto[] boletos = sistema.getGestorBoletos().getBoletos(); //
        if (boletos != null) {
            for (Boleto b : boletos) {
                if (b != null) comboBoxInputBoleto.getItems().add(b.getIdBoleto()); //
            }
        }
    }
    ...
    void onBuscarClick(ActionEvent event) {
        ...
        if (buscarPorPasajero) {
            valorBusqueda = fieldInputPasajero.getText().trim();
        } else {
            valorBusqueda = comboBoxInputBoleto.getValue();
        }
    ...
    // --- Métodos crearTarjetaBoleto, crearInfoLabel, popups y auxiliares ---
    // Son idénticos a los de EmpleadoBoletosConsultarController
    ...
    void onBackClick(ActionEvent event) {
        // Vuelve a la pantalla de opciones (mis_boletos_opciones.fxml)
        try {
            FXMLLoader loader = new FXMLLoader(Aplicacion.class.getResource("/mis_boletos_opciones.fxml")); //
